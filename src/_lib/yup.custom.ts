...
const jpConfig = {
  ...
  number: {
    min: (prm: MessageParams & {min: number}) => `${labelText(prm)}${prm.min}以上で入力してください。`,
    max: (prm: MessageParams & {max: number}) => `${labelText(prm)}${prm.max}以下で入力してください。`,
    ...
    positive: (prm: MessageParams & {more: number}) => `${labelText(prm)}正数で入力してください。`,
    negative: (prm: MessageParams & {less: number}) => `${labelText(prm)}負数で入力してください。`,
